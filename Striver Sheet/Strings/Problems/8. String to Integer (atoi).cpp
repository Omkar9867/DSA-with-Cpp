...
int myAtoi(std::string s) {
    ...
    while (i < s.size() && isdigit(s[i])) {
        ...
        if (sign * count > INT_MAX) return INT_MAX;

        // Clamp to INT_MIN if underflow occurs
        if (sign * count < INT_MIN) return INT_MIN;
...
int main(){
    std::string inputArr = "LVIII";
    // int result = myAtoi(inputArr);

    // std::cout << "Result: " << result << std::endl;
    std::cout << "Result: " << typeid(inputArr).name() << std::endl;
    return 0;
}
